...
FLOWS: Dict[str, Dict[str, Any]] = {
    "build": {
        ...
        "stages": [
            ...
            _s("chunk_index", "청킹 · FTS 색인", "헤딩 단위 섹션 → 문단 경계 분할(오버랩) → chunks + chunks_fts(BM25, 조사 제거 토큰·bigram).",
               "청크 크기가 검색 정밀도/컨텍스트 토큰/벡터 메모리를 좌우. 값 변경 시 전체 리빌드.",
               [], ["chunk_max_chars", "chunk_overlap_chars"], ["build --full"], tuning_stage="chunk_index", module="corpus.py · store.py", io="Document → Chunk[] → FTS5"),
            _s("embed", "벡터 임베딩", "청크 텍스트를 임베딩해 embeddings 테이블에 저장. hash 임베더는 IDF 를 전체 빌드에서 적합.",
               "벡터 채널 recall 의 원천. hash 는 오프라인·비의미적, voyage/st 는 의미 검색. 메모리 = 청크×dim×4B.",
               ["embed", "idf_refit_incremental"], ["embed_provider", "embed_model", "embed_dim", "embed_batch"],
               ["build --no-embed", "build --idf-refit-incremental", "models set embed_provider=voyage"], tuning_stage="embed", module="providers.py", io="Chunk[] → float32[dim]"),
            _s("graph_build", "그래프 추출", "규칙(사전·정규식) 및/또는 LLM 으로 엔티티·관계·멘션 추출 → degree → 노드 doc_refs → (전체 빌드) 커뮤니티 → (옵션) 요약.",
               "그래프 채널·위키·엔티티 상세의 원천. 규칙은 무료·결정적, LLM 은 청크당 1회 호출(토큰↑, budget 으로 제한).",
               ["rule_graph", "llm_graph", "communities", "community_summary", "incremental_communities"],
               ["llm_graph_budget", "llm_graph_min_chars", "llm_roles.extract", "llm_roles.summary"],
               ["build --llm-graph", "build --no-rule-graph", "build --community-summary", "entity <name>", "graph"],
    ...
    "query": {
        ...
        "stages": [
            ...
            _s("context", "컨텍스트 구성", "상위 top_k_final 청크를 [C#] 블록으로 조립. 중복 제거(오버랩·유사 문단)·문장 압축·인접 청크·그래프 관계 첨부.",
            ...
            _s("forensic", "포렌식 (자동)", "근거 부족·낮은 groundedness 시 trace 를 진단 규칙으로 분석해 findings/suggestions 를 forensics 테이블에 기록. 누적 → 제안.",
               "실패 원인 추적과 자가진화 데이터 확보.", ["forensic_auto"], [], ["forensic last", "forensic <request_id>", "forensic summary"],
               trace=["forensic"], tuning_stage="forensic", module="forensic.py"),
