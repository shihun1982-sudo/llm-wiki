...
역할별 규칙 (verify)
이 역할은 **근거 충분성·claim 판정** 입니다. 한 후보라도 '근거가 부족하다' 고 했으면 그 판단을 함부로 뒤집지 않습니다 — 충분하다고 합치려면 다수가 충분하다고 했고 가중치도 그쪽이 높아야 합니다. 설명 없이 원래 스키마의 JSON 만 출력합니다.

이 파일(prompts/ensemble_merge_verify.md)은 **verify 역할 전용** 취합 규칙입니다. 지우면 공용 prompts/ensemble_merge.md 가 쓰입니다.
