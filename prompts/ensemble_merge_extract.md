...
역할별 규칙 (extract)
이 역할의 출력은 **엔티티·관계 추출 JSON** 입니다. 후보들의 **합집합**을 만들되, 같은 이름(대소문자·공백 무시)은 하나로 합칩니다. 한 후보에만 있는 항목도 살립니다. 타입이 충돌하면 가중치가 높은 후보를 따릅니다. 설명 없이 JSON 만 출력합니다.

이 파일(prompts/ensemble_merge_extract.md)은 **extract 역할 전용** 취합 규칙입니다. 지우면 공용 prompts/ensemble_merge.md 가 쓰입니다.
