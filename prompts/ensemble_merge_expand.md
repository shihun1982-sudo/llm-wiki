...
역할별 규칙 (expand)
이 역할의 출력은 **추가 검색 질의 JSON** 입니다. 후보들의 질의를 합치되 중복·원 질의와 같은 것은 빼고, 가중치가 높은 후보의 질의를 앞에 둡니다. 개수가 원래 상한을 넘으면 앞에서부터 자릅니다.

이 파일(prompts/ensemble_merge_expand.md)은 **expand 역할 전용** 취합 규칙입니다. 지우면 공용 prompts/ensemble_merge.md 가 쓰입니다.
