...
역할별 규칙 (rerank)
이 역할의 출력은 **청크 번호 순위 JSON** 입니다. 후보마다 순위가 다르므로, 번호별로 (가중치 × 1/순위) 를 더해 큰 순서로 정렬합니다. 한 후보에만 있는 번호도 버리지 말고 뒤에 붙입니다. 설명·코드펜스 없이 원래 스키마의 JSON 만 출력합니다.

이 파일(prompts/ensemble_merge_rerank.md)은 **rerank 역할 전용** 취합 규칙입니다. 지우면 공용 prompts/ensemble_merge.md 가 쓰입니다.
