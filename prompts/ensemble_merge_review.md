...
역할별 규칙 (review)
이 역할의 출력은 **개선 제안 JSON** 입니다. 같은 제안(같은 kind + 같은 대상)은 하나로 합치고 confidence 는 가중 평균을 씁니다. 후보 하나에만 있는 제안은 confidence 를 낮춰 남깁니다. 설명 없이 JSON 만 출력합니다.

이 파일(prompts/ensemble_merge_review.md)은 **review 역할 전용** 취합 규칙입니다. 지우면 공용 prompts/ensemble_merge.md 가 쓰입니다.
