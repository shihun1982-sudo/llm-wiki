...
> 2026-09-20 22:01 기준 · Python 3.14.7 · 모두 격리 임시 환경에서 실행 (실제 색인·설정은 건드리지 않는다)

| 무엇 | 명령 | 결과 |
|---|---|---|
| 단위 테스트 | `python -m unittest discover -s tests` | **773/773 통과** (159s) |
...
| 세 창구 동작 동등성 (CLI 프로세스 · Web · MCP) | `python tools/verify/verify_tri_surface.py --port 8879` | **65/65 통과** (6s) |
...
| Web API 전수 | `python tools/verify/verify_web.py` | **379/379 통과** (39s) |
| MCP 종단 | `python tools/verify/verify_mcp.py --quick` | **129/129 통과** (6s) |
...
| 앙상블 편집기 (실제 클릭) | `python tools/verify/verify_ensemble_ui.py` | **28/28 통과** (9s) |
...
| 타임아웃 · 취소 내성 | `python tools/verify/verify_timeouts.py --port 8907` | **47/47 통과** (136s) |
| 다중 클라이언트 혼합 부하 | `python tools/verify/verify_soak.py --port 8971 --seconds 60 --clients 16` | **10/10 통과** (64s) |
| 무작위 입력 내성 | `python tools/verify/verify_monkey.py` | **OK** (658s) |
<!-- /VERIFY_ALL_TABLE -->
