...
검증 결과(CLI·Web·UI·브라우저)와 수정 결함은 [VERIFICATION_0915.md](../2026-09-15/VERIFICATION_0915.md), 설정 위치 총람은 [BRINGUP_GUIDE.md §3.2](../../BRINGUP_GUIDE.md).
...
### 10.3 검증
`tests/test_analysis.py` 6개(전체 87개 통과), 하네스 CLI(`query --analyze`, `analyze last|--print|--json|없는 id`)·Web(`overrides.analysis_mode` → `/api/analysis` json/md/download/404, MCP `wiki_analysis`) — [VERIFICATION_0915.md](../2026-09-15/VERIFICATION_0915.md) §8.

### 9.3 검증
`tests/test_rag_federation.py` 7개(전체 87개 통과), 하네스 `verify_cli.py`(mcp-source tools/retrieve/federated, query ±external-rag, stdio 페더레이션)·`verify_web.py`(`/api/mcp_sources` retrieve/tools/federated, external_rag 오버라이드, `/mcp` 의 `mock__search`·`wiki_external_search`·깊이 헤더 가드) — [VERIFICATION_0915.md](../2026-09-15/VERIFICATION_0915.md) §7. 발견·수정: 자기 자신/상호 expose 시 tools/list 무한 재귀(→ 깊이 헤더), 단일 리스트인 외부 채널이 RRF 에서 항상 밀리는 문제(→ inject + consensus 보정).
