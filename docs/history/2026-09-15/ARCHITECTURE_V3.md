...
> v2(2026-09-11) 이후 추가된 v3 기능(문서 계약·provenance·임베딩 캐시/재개·규칙/LLM 질의 확장·근거 판정/fallback·claim 검증·포렌식·메모리·trial·프리셋·headless 에이전트·MCP 소스·워크플로 UI·테마)을 포함한 최종 구조. 단계별 CLI 예제는 [CLI_FLOWS.md](../../CLI_FLOWS.md), 포팅은 [BRINGUP_GUIDE.md](../../BRINGUP_GUIDE.md), 이전 구조는 [ARCHITECTURE_V2.md](../2026-09-13/ARCHITECTURE_V2.md).
...
| 인터페이스 | `cli.py` `web/server.py` `web/static/*` `mcp.py` `architecture.py` | argparse CLI(<!--live:cli-->48 명령) · HTTP API(<!--live:api-->112 경로) + 워크플로 UI(로그인 페이지, 단계 확인 모달) · MCP 도구 <!--live:mcp-->20종 · 구조 레지스트리 — 세 창구의 대조와 동등성은 [SURFACE_AUDIT_0920.md](../../SURFACE_ALIGNMENT.md) |
...
- **CLI**: `build[ fts|vector|graph]|health|query|search|eval|trial|graph|entity|corpus|embed|rules|pin|precompute|forensic[ expect]|fusion|memory|time|preset|prompts|logs|requests|config|models|tuning|arch|evolve|wiki|docs|stats|system|maintenance|watch|mcp-source|mcp|serve|users|security|apikey|snapshot` — 상세는 CLI_FLOWS.md. 전역 `--user` 로 실행자 로그인(권한 게이트, SECURITY.md §5).
...
- **MCP** (MCP.md): 전송 stdio(같은 PC) / **Streamable HTTP**(`serve` 의 `POST /mcp`, 또는 `mcp --transport http`; Bearer API 키·세션 쿠키·익명) / 브리지(`mcp --connect URL`). 도구 `wiki_query(question, k, mode=fast|normal|deep, doc_types, preset)` · `wiki_search` · `wiki_related(text, doc_types)` · `wiki_doc(id)` · `wiki_entity` · `wiki_propose(kind, payload)` · `wiki_feedback(query_id, ±1, note)` · `wiki_forensic(request_id, expected_docs, expected_terms)` · `wiki_status`. 색인을 직접 바꾸는 도구는 없음(제안·피드백·진단 기록만, HITL). 여러 클라이언트는 서버 락으로 직렬화.
- **Web/CLI/MCP 권한·진행 표시** (SECURITY.md): 모든 요청은 `auth.py` 가 식별(프록시 헤더 SSO → Bearer API 키 → 세션 쿠키 → 익명 게스트) → 작업 등급 7단계(read/run/edit/index/rebuild/admin/destructive) × **권한 표(permissions: 등급별 최소 역할 + op 오버라이드, admin 편집)** 로 역할 6단계(viewer<class3<class2<class1<builder<admin) 검사 → 확인/문구/재인증 → 실행 → 감사 로그. CLI 는 `_cli_gate`(실행자 = `--user`/`LLMWIKI_USER`/`LLMWIKI_API_KEY`/`cli.default_role`)가 같은 표를 쓴다. 오래 걸리는 job/질의는 `progress.py` 레지스트리에 단계·진도율·LLM 대기·재시도 횟수를 기록하고 `/api/jobs/<id>`·`/api/progress/<token>`(서버 락 밖) 로 폴링한다.
- **상세 분석 모드 (2026-09-15, ANALYSIS_MODE.md)**: 토글 `analysis_mode` 면 `QueryEngine.run` 이 Profiler 를 debug 2 로 만들고(단계별 debug + 프롬프트/응답 샘플), 요청 기록 후 `analysis.run_for_result` 가 trace/result 를 리포트(dict → markdown/json, `logs/analysis/req_<id>.*`)로 저장해 `result.analysis` 에 경로·렌즈 상위 소견을 붙인다. 저장된 요청도 `analysis.analyze(pipe, request_id)` 로 생성. 렌즈 규칙은 trace 수치(단계 ms·토큰·채널 겹침·리랭크 이동·컨텍스트 탈락·판정·claim)를 보고 `architecture.registry()` 의 단계↔튜닝 키와 현재값을 조절점으로 붙인다. 인터페이스: `analyze` CLI · `GET /api/analysis` · MCP `wiki_analysis`.
- **다른 RAG 연동·MCP 확장 (2026-09-15, RAG_FEDERATION.md)**: `mcp_client.py` 가 외부 소스를 stdio/http/rest 로 열고(`open_source`, 풀), `retrieve` 매핑으로 결과를 가상 청크 `ext:<source>:<id>` 로 만들어 `query_engine._retrieve` 의 `external_rag` 단계가 채널 `ext_<source>` 로 융합 → `external_inject`(리랭크 후보 보장) → 리랭크·컨텍스트(인용, `hits.external`). `mcp.py` 는 built-in 도구 11개 + 플러그인(`plugins/mcp_tools/*.py`, `register_tool`) + 페더레이션(`expose` 한 원격 tool 을 `<source>__<tool>` 로 중계, 재귀 방지 헤더/환경변수)을 하나의 `tools/list`·`tools/call` 로 낸다. 토글 `external_rag`·`mcp_federation`, 튜닝 `channel_w_external`·`external_rag_k`·`external_rag_inject`.
- **2026-09-14 추가 단계** (§4/§5 표의 보완): 빌드 `build_channel`(fts→`reindex_fts` / vector→`embed` / graph→`graph_build`; 다른 채널 불변, `counts_before/after`·verify), 질의 `doc_expand`(리랭크 후·context 전; 튜닝 `doc_expand_*`), LLM 호출의 재시도(`BaseLLM.complete`, transient 오류만; `llm_retries`/`agents.json retries`)와 실패 incident → `result.llm_report`/빌드 alerts, 기대 결과 포렌식 `forensic.trace_expectation`(`QueryEngine.rounds` 캡처 + `record_request=False` 재실행). 근거: IMPLEMENTATION_PLAN_0914.md.
