...
| **2026-09-24 (Knowledge)** | **그래프 진단을 소견·처방으로**(13종, 증거→원인→처방 조각→확인) · **그래프 탭** 용어 설명·전체 규모 먼저·보기 모드 4종·무리 상세(세 창구) · 사전 매처 **단어 경계·대소문자**(실데이터 허브 1~5위가 `corpus_d[ir]s` 류 오탐) | [GRAPH_KNOWLEDGE_PLAN_0924.md](2026-09-24/GRAPH_KNOWLEDGE_PLAN_0924.md)(조사한 사실 7건 · 택하지 않은 대안 6개 · 검증) |
| **2026-09-24** | 앞 회차의 **사후 코드 리뷰와 전체 검증** — 의도대로 구현됐는지, 부수 피해는 없는지 | [CODE_REVIEW_0924.md](2026-09-24/CODE_REVIEW_0924.md)(결함 15건: 정상 종료 때 원장이 잘림 · 테스트가 실사용 원장에 회당 700~800줄 · CLI 질의에 상세 링크 없음 · 재현 하네스가 거짓 OK · 지표의 분모 오류 · 문서-코드 불일치 · **여러 프로세스가 같은 원장 파일에 쓰면 줄이 깨짐**(Windows `O_APPEND` 는 프로세스 간 원자적이지 않다 → 파일 잠금) · 짧게 사는 CLI 프로세스가 종료 시 버퍼를 버림(`atexit`) · 세 창구 동시 부하 시험 부재(`verify_three_surface_load.py` 신설) · cp949 콘솔에서 테스트·검증이 거짓 FAIL(목업 자식의 인코딩) · **끊긴 클라이언트의 요청이 대기열을 30분 차지**(멍키 하네스가 본문을 버려 못 보던 것 → 소켓 EOF 감지) · **stderr 파이프가 막히자 배타 잠금을 쥔 채 서버가 멎음**(하네스가 파이프를 안 읽음 + Web 콘솔 CLI 오류가 stderr 로) · 위키 페이지 이름 미검증(500·경로 탈출) · 새 요청 행에 IP·에이전트 누락(09-23 회귀) · 테스트가 실사용 logs/ 오염. §5 에 재발 방지 규칙 11가지) |
