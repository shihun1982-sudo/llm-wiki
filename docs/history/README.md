...
| **2026-09-24** | 앞 회차의 **사후 코드 리뷰와 전체 검증** — 의도대로 구현됐는지, 부수 피해는 없는지 | [CODE_REVIEW_0924.md](2026-09-24/CODE_REVIEW_0924.md)(**가장 최근** — 결함 10건: 정상 종료 때 원장이 잘림 · 테스트가 실사용 원장에 회당 700~800줄 · CLI 질의에 상세 링크 없음 · 재현 하네스가 거짓 OK · 지표의 분모 오류 · 문서-코드 불일치 · **여러 프로세스가 같은 원장 파일에 쓰면 줄이 깨짐**(Windows `O_APPEND` 는 프로세스 간 원자적이지 않다 → 파일 잠금) · 짧게 사는 CLI 프로세스가 종료 시 버퍼를 버림(`atexit`) · 세 창구 동시 부하 시험 부재(`verify_three_surface_load.py` 신설) · cp949 콘솔에서 테스트·검증이 거짓 FAIL(목업 자식의 인코딩). §5 에 재발 방지 규칙 9가지) |
